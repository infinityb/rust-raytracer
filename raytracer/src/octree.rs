...
impl<T> FromIterator<T> for Octree<T> where T: PartialBoundingBox {
    fn from_iter<I>(iterator: I) -> Self where I: IntoIterator<Item=T> {
        ...
        let bounds = BBox::from_union(finites.iter())
...
impl OctreeNode {
        ...
        for x in 0..2 {
            for y in 0..2 {
                for z in 0..2 {
                    ...
                    let child_bbox = BBox {
                        min: Vec3 {
                            x: self.bbox.min.x + f64::from(x) * len.x / 2.0,
                            y: self.bbox.min.y + f64::from(y) * len.y / 2.0,
                            z: self.bbox.min.z + f64::from(z) * len.z / 2.0
                        },
                        max: Vec3 {
                            x: self.bbox.max.x - f64::from(1 - x) * len.x / 2.0,
                            y: self.bbox.max.y - f64::from(1 - y) * len.y / 2.0,
                            z: self.bbox.max.z - f64::from(1 - z) * len.z / 2.0,
                        }
                    };
    ...
    pub fn insert(&mut self, index: usize, object_bbox: BBox) -> () {
        ...
        // Empty leaf node
        if self.is_leaf() && self.leaf_data.is_empty() {
            self.leaf_data.push(OctreeData { index: index, bbox: object_bbox });
            return;
        }
    ...
    fn next(&mut self) -> Option<&'a T> {
        ...
        loop {
            ...
            if let Some(leaf_iter) = self.leaf_iter.as_mut() {
                if let Some(val) = leaf_iter.find(|x| x.bbox.intersects(ray)) {
