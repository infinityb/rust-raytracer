...
pub fn get_scene() -> Scene {
    ...
    prims.extend(IcosahedronBuilder::new().radius(4.0).build().map(|tri| box tri as Box<Prim+Send+Sync>));
    prims.push(box Sphere { center: Vec3 { x: 0.0, y: 0.0, z: 1.0 }, radius: 0.15, material: box shiny });
