...
pub fn get_scene() -> Scene {
    ...
    lights.push(Box::new(SphereLight {
        position: Vec3 { x: 200.0, y: -200.0, z: 100.0 },
        color: ColorRGBA::white(),
        radius: 40.0,
    }));
    lights.push(Box::new(SphereLight {
        position: Vec3 { x: -95.0, y: 20.0, z: 170.0 },
        color: ColorRGBA::new_rgb(0.5, 0.5, 0.3),
        radius: 15.0,
    }));

    let red = CookTorranceMaterial {
        k_a: 0.1, k_d: 0.4, k_s: 0.5, k_sg: 0.5, k_tg: 0.0,
        gauss_constant: 5.0,
        roughness: 0.05,
        glossiness: 0.0,
        ior: 0.98,
    ...
    let green = CookTorranceMaterial {
        k_a: 0.0, k_d: 0.4, k_s: 0.6, k_sg: 0.7, k_tg: 0.0,
        gauss_constant: 50.0,
        roughness: 0.3,
        glossiness: 0.0,
        ior: 1.5,
    ...
    let shiny = CookTorranceMaterial {
        k_a: 0.0, k_d: 0.2, k_s: 0.7, k_sg: 1.0, k_tg: 0.0,
        gauss_constant: 25.0, roughness: 0.01, glossiness: 0.0, ior: 0.2,
        ambient: ColorRGBA::white(),
        diffuse: ColorRGBA::new_rgb(0.9, 0.9, 0.1),
        specular: ColorRGBA::new_rgb(0.9, 0.9, 0.1),
        transmission: ColorRGBA::black(),
        diffuse_texture: None
    };
    ...
    let bunny = ::util::import::from_obj(Box::new(red), false, "./docs/assets/models/bunny.obj").expect("failed to load obj model");
