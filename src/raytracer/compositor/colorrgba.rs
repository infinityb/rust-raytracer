...
impl ColorRGBA<u8> {
    pub fn new_rgb_clamped(r: f64, g: f64, b: f64) -> ColorRGBA<u8> {
        ...
        ColorRGBA::new_rgb(
            clamp((r * max_color as f64).round() as int, min_color as int, max_color as int) as u8,
            clamp((g * max_color as f64).round() as int, min_color as int, max_color as int) as u8,
            clamp((b * max_color as f64).round() as int, min_color as int, max_color as int) as u8)
    }
}
