...
pub fn from_obj(material: CookTorranceMaterial /*Box<Material>*/,
    ...
    let total_bytes = match lstat(&filename) {
        Ok(stat) => stat.size,
    ...
    for line_iter in file.lines() {
        ...
        match tokens[0].as_slice() {
            ...
            "vn" => {
                ...
                normals.push(Vec3 {
                    x: tokens[1].parse().unwrap() * normal_scale,
                    y: tokens[2].parse().unwrap() * normal_scale,
                    z: tokens[3].parse().unwrap() * normal_scale
                });
            },
            "f" => {
